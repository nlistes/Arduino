...
#define PERIOD 1000000 / (2 * FREQ) //The Timerone PWM period in uS, 60Hz = 8333 uS
...
volatile byte state = B11111111;  // controls what interrupt should be attached or detached while in the main loop
...
unsigned int onTime = 3000;  // the calculated time the triac is conducting
...
void setup()
{
    ...
    Timer1.initialize(PERIOD);  // 10mS - maiņstrāvas pusperiods
    //  Timer1.disablePwm(9);
    ...
    if (hexValue < 0)
      {
        ...
        if (state==B00000011)  //its before the turn on time
          ...
          if (state==B00000010) //its after turn on time
            {
...
// function to be fired at the zero crossing.  This function keeps the AC_PIN full on or full off if we are at max or min
